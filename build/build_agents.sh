...
if [ -z "$AGENT_NUMBER" ] || [ "$AGENT_NUMBER" -lt 20 ]; then
  ...
  # Process Cancer Biomarker discovery Supervisor agent template
...
if [ -z "$AGENT_NUMBER" ]; then
  ...
  agent_dir=$(find . -maxdepth 1 -type d -name "${AGENT_NUMBER}-*" | head -1)
  if [ -z "$agent_dir" ]; then
  ...
  for agent_file in $(find "${agent_dir}" -type f -name "*.yaml"); do
    if [ -f "${agent_file}" ]; then
      echo "Found agent file: ${agent_file}"
      agent_name=$(basename "${agent_file}" .yaml)
      echo "Packaging agent: ${agent_name}"
...
if [ -z "$AGENT_NUMBER" ]; then
  ...
  if [ -f "agents_catalog/10-SEC-10-K-agent/action-groups/SEC-10-K-search/docker/sec-10-k-docker.zip" ]; then
  ...
  if [ -f "agents_catalog/15-clinical-study-research-agent/lambdalayers/matplotlib.zip" ]; then
